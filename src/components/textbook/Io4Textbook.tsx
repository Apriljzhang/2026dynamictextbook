...
export function Io4Textbook() {
  ...
  const [sectionId, setSectionId] = useState(io4LectureSections[0]?.id ?? "io4-01");
  ...
  return (
    <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,240px)_minmax(0,1fr)] lg:items-start">
      <nav className="lg:sticky lg:top-28">
        <p className="text-xs uppercase tracking-[0.14em] text-[color:var(--color-muted)]">目录</p>
      ...
      <div>
        <div className="rounded-sm border border-[color:var(--color-border)] bg-[color:var(--color-bg)] px-4 py-3 text-sm text-[color:var(--color-carbon)]">
          <p className="text-xs uppercase tracking-[0.12em] text-[color:var(--color-muted)]">指定讲义</p>
          <p className="mt-1 font-medium text-[color:var(--color-navy)]">{IO4_SOURCE.courseTitle}</p>
          <p className="mt-1 text-xs text-[color:var(--color-muted)]">文件：{IO4_SOURCE.fileName}</p>
          <a
        ...
        <div className="mt-8 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="text-xs text-[color:var(--color-muted)]">
            当前小节：<span className="text-[color:var(--color-carbon)]">{section.n} · {section.title}</span>
          </p>
        </div>
